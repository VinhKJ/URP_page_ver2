...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const { data: user, error, isLoading } = useQuery<PublicUser | undefined, Error>({
    queryKey: ["/api/user"],
  ...
  const registerMutation = useMutation({
    ...
    onSuccess: (user: PublicUser) => {
      ...
      toast({
        ...
        description: `Chào mừng ${user.username} đến với URP!`,
      });
    },
    onError: (error: Error) => {
      toast({
        title: "Đăng ký thất bại",
        description: error.message || "Tên đăng nhập đã tồn tại hoặc thông tin không hợp lệ",
        variant: "destructive",
      });
    },
  });
